...
- Edit and expand existing bridge/agent/control files before creating new small files.
...
|---|---|---|---|
| `CORE-B01` | Task transcription from old sources into CORE | Achilles | Active; extract one batch per run |
| `CORE-B02` | Project-function divvy by Z01-Z06 and support lane | Neo | Active; assign bracket + floor owner |
...
- Active automation surface has been consolidated around CORE daily orchestration plus weekly deletion/publish readiness review.
- OpenAI Platform target observed: organization `Romer Industries`, default project `Default project`. No key, secret, deployment or model configuration change was required or performed.

...
1. `data/intersol/facility/intersol_facility_finalization_matrix_v0_1.json`
2. `data/infrastructure/doctrine/t1_25_design_doctrine_v0_1.json`
3. `data/intersol/watchtower/optimization/watchtower_optimization_envelope_v0_1.json`
4. `data/intersol/web4/infrastructure_coin_provenance_map_v0_1.json`
5. `schemas/intersol_multiobjective_optimization_receipt_fields.json`
...
Hard safety/compliance constraints are non-tradeable. Approximate 25% service/interface reserve or 75/25 durable/experimental visual balance may be used as explicit scenario/design heuristics only where technically and economically justified.
...
Provisional analysis defaults now allow deterministic candidate generation:

- A_zeta exploratory `0.20..0.50`, preferred `0.30..0.45`;
- C_zeta exploratory `0.55..0.90`, preferred `0.65..0.80`;
- on current Pad004 these arithmetic transforms correspond to A preferred `29.80..38.05m` and C preferred `49.05..57.30m`, but no anchor height is selected by those ranges;
...
Optimisation uses hard-constraint pruning then Pareto ranking. A safety/compliance failure cannot be hidden by weighted aesthetics, ecology, energy or cost.
